...
entries = []
add_another = st.checkbox("Add Another Entry")

if add_another:
    full_payment_cash_in = st.number_input("Enter Full Payment Cash-in (€):", min_value=0.0, step=0.01, key="full_payment")
    mrp = st.selectbox("Select MRP (€):", options=[119, 349, 649, 1199, 1999], key="mrp")
    deal_source = st.selectbox("Select Deal Source:", options=["PM-Search", "PM-Social", "Organic", "Others", "Referral", "Events", "Goldmine", "DP"], key="deal_source")
    if st.button("Calculate Incentive", key="price_control"):
        ...
        entries.append(price_control_incentive)
...
if st.button("Calculate Total Incentive"):
    ...
    total_price_control_incentive = sum(entries)
    additional_incentive = (d0_cases * 300) + (within_window_cases * 4000) + (self_gen_cases * 3000)
    total_incentive = total_upfront_incentive + total_price_control_incentive + additional_incentive
